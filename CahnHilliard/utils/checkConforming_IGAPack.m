function [ PHTelem, dimBasis, quadList, maxLevel ] = checkConforming_IGAPack( PHTelem, dimBasis, patchBoundaries, p, q, quadList )
...
%calculates the maximum level of refinement along the interface
...
while keepChecking
    ...
    for boundaryIndex = 1:numBoundaries
        ...
        for indexPatch=1:length(patchAList)
            ...
            for i=1:length(elementsA{1})
                if PHTelem{patchA}(elementsA{1}(i)).level>maxLevel
                    maxLevel = PHTelem{patchA}(elementsA{1}(i)).level;
                end
            end
            
            for i=1:length(elementsB{1})
                if PHTelem{patchB}(elementsB{1}(i)).level>maxLevel
                    maxLevel = PHTelem{patchB}(elementsB{1}(i)).level;
                end
            end
