function  plotSolPHUTCahnHilliardSphere(PHTelem, GIFTmesh,  p, q, sol )
...
for patchIndex = 1:length(PHTelem)
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            tempDisp=sol(PHTelem{patchIndex}(i).nodesGlobal);
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    [coord] =paramMapSemiSphere3(PHTelem, patchIndex,i,uref(ii),vref(jj));
                    coords(ii,jj,:)=[coord(1),coord(2),coord(3)];
