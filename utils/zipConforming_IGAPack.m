function [ PHTelem, sizeBasis ] = zipConforming_IGAPack( PHTelem, dimBasis, patchBoundaries, p, q )
...
for patchIndex = 1:numPatches
    for elemIndex = 1:length(PHTelem{patchIndex})
        ...
        PHTelem{patchIndex}(elemIndex).modifiedC=PHTelem{patchIndex}(elemIndex).C;
        PHTelem{patchIndex}(elemIndex).polyDegree = p*ones(1,length(PHTelem{patchIndex}(elemIndex).nodesGlobal));
    end
end
