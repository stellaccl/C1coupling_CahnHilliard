function  plotBasis1D_GIFTmesh(PHUTelem,GIFTmesh,p,indexBasis)

numPts = 100; %number of plot points to use on each edge
fudge = 0;
    ...
    for indexElem = 1:length(PHUTelem{indexPatch})
        ...
        if isempty(PHUTelem{indexPatch}(indexElem).children)
            ...
            if any(localIndex)
                ...
                for ii=1:numPts
                    cR(ii) = (PHUTelem{indexPatch}(indexElem).C(localIndex,:))*R(ii,:)';
                    
                    dR = (PHUTelem{indexPatch}(indexElem).C(localIndex,:))*dRdu(ii,:)';
