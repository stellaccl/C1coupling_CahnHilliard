function PlotDiff_circle_biharmonic_degElev( PHTelem, GIFTmesh,  p, q, sol)
...
%1D bernstein polynomials evaluated at the Gauss points on the master element
[B_uDE,~] = bernstein_basis(uref,p+1);
[B_vDE,~] = bernstein_basis(vref,q+1);

BuvDE = zeros(numPts, numPts, (p+2)*(q+2));
        ...
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    disp(ii,jj)=(R'*tempDisp)-exactSol;
                end
            end
            %  maxZ = max([maxZ; disp(:)]);
