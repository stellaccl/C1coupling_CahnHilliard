function [ stiff, rhs, bcdof, bcval] = imposeDirichletPTestPHUT_billinear2patch_degElev(stiff, rhs, PHUTelem, p, q, bound_disp,patchBoundaries,type2Basis)
...
% set dirichlet boundary based on nodes support
...
for boundaryIndex = 1:numBoundaries
    ...
    for i=1:length(elementsA)
        ...
        bcdof_left = [bcdof_left, PHUTelem{patchA}(elementsA(i)).tempNodesGlobal(left_nodes)];
        
    end
    
    for i=1:length(elementsB)

        bcdof_right = [bcdof_right, PHUTelem{patchB}(elementsB(i)).tempNodesGlobal(right_nodes)];
    end
end
