function [ GIFTmesh] = genGIFTmesh_thinShell( knotU, knotV, coefs, p, q, numberElementsU, numberElementsV )
...
for j=1:lenV %for each node in the y direction
    for i=1:lenU % for each node in the x direction
        ...
        coordinates(index,:) = [coefs(1,i,j)./coefs(4,i,j), coefs(2,i,j)./coefs(4,i,j), coefs(3,i,j)./coefs(4,i,j),coefs(4,i,j)]; %put the (i,j) node in the coordinate array
