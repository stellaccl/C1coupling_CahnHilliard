function  plotSolPHTElasticMPVM_modifiedC( sol0, PHTelem, GIFTmesh, p, q, Cmat, factor )
...
for indexPatch = 1:numPatches
    for i=1:length(PHTelem{indexPatch})
        if isempty(PHTelem{indexPatch}(i).children)
            ...
            nument = size(PHTelem{indexPatch}(i).modifiedC,1); %number of basis functions with support on current knotspan
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    cR = PHTelem{indexPatch}(i).modifiedC * squeeze(Buv(ii,jj,:));
                    cdRdx = PHTelem{indexPatch}(i).modifiedC * squeeze(dBdu(ii,jj,:))*2/(xmax-xmin);
                    cdRdy = PHTelem{indexPatch}(i).modifiedC * squeeze(dBdv(ii,jj,:))*2/(ymax-ymin);
                   ...
                   % stressVM{jj,ii} = sqrt(stressvect{jj,ii}(1)^2 - stressvect{jj,ii}(1)*stressvect{jj,ii}(2) + stressvect{jj,ii}(2)^2 +3*stressvect{jj,ii}(3)^2);
                    stressVM{jj,ii} =stressvect{jj,ii}(2);
                end
            end
