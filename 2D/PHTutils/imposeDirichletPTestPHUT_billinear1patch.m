function [ stiff, rhs, bcdof, bcval] = imposeDirichletPTestPHUT_billinear1patch(stiff, rhs, PHUTelem, p, q, bound_disp)
...
for indexElem=1:length(PHUTelem{1})
    
    if isempty(PHUTelem{1}(indexElem).neighbor_left)
        bcdof_left = [bcdof_left, PHUTelem{1}(indexElem).nodesGlobal(left_nodes)];
    end
    
    if isempty(PHUTelem{1}(indexElem).neighbor_right)
        bcdof_right = [bcdof_right, PHUTelem{1}(indexElem).nodesGlobal(right_nodes)];
    end
