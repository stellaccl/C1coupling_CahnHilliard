function  plotSolCahnHilliardShell2(PHTelem,basisFunPlotSol, p, q, sol )
...
for patchIndex = 1:length(PHTelem)
    for elemIndex=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(elemIndex).children)
            ...
            tempDisp=sol(PHTelem{patchIndex}(elemIndex).nodesGlobal);
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    %[coord,dxdxi] = paramMapShell(GIFTmesh{patchIndex}, uref(ii),vref(jj), xmin, ymin, xmax, ymax);
                    
                    tempCoord= basisFunPlotSol.coords{patchIndex,elemIndex,ii,jj};
                    coords(ii,jj,:)=[tempCoord(1),tempCoord(2),tempCoord(3)];
                    R = basisFunPlotSol.R{patchIndex,elemIndex,ii,jj};
            ...
            surf(coords(:,:,1), coords(:,:,2),coords(:,:,3),disp,'FaceColor','interp','EdgeColor','none')
            hold on
