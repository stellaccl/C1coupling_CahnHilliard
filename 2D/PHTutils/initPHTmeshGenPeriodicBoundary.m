function [ PHTelem, dimBasis, tempDimBasis] = initPHTmeshGenPeriodicBoundary( p,q, numElemU, numElemV )
%from the mother fucking mix functions directory
...
% %plot the basis functions in 1D
% numPts = 101;
% evalPts = linspace(-1,1,numPts);
% B = bernstein_basis(evalPts,p);
% plotKnots = [0,0.2,0.4,0.6,0.8,1];
% 
% figure
% for indexElem=1:5
%    % size(C_u(:,:,indexElem))
%    % size(B)
%     basisVal = C_u(:,:,indexElem)*B';
%     xVals = linspace(plotKnots(indexElem),plotKnots(indexElem+1),numPts);
%     plot(xVals, basisVal)
%     hold on
% end
% pause
