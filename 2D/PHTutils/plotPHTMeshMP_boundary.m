function  plotPHTMeshMP_boundary( PHTelem, GIFTmesh )
...
for patchIndex = 1:numPatches
    ...
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            up=PHTelem{patchIndex}(i).neighbor_up;
            down=PHTelem{patchIndex}(i).neighbor_down;
            left=PHTelem{patchIndex}(i).neighbor_left;
            right=PHTelem{patchIndex}(i).neighbor_right;
            ...
            if isempty(up)
                line(coord_north(:,1), coord_north(:,2), 'Color','k', 'LineWidth',2)
            end
            
            
            if isempty(right)
                line(coord_east(:,1), coord_east(:,2), 'Color','k', 'LineWidth',2)
            end
            
            if isempty(down)
                line(coord_south(:,1), coord_south(:,2), 'Color','k', 'LineWidth',2)
            end
            
            if isempty(left)
                line(coord_west(:,1), coord_west(:,2), 'Color', 'k', 'LineWidth',2)
            end
            
             coords = [coord_south; coord_east; coord_north; coord_west];
             line(coords(:,1), coords(:,2), 'Color','k', 'LineWidth',0.5)
