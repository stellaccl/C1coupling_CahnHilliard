function  [l2relerr, h1relerr]=calcErrorNormsPH_modifiedC( sol0, PHTelem, GIFTmesh, p, q, Cmat, Emod, nu, rad, tx )
        ...
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for jj=1:numGaussY
                for ii=1:numGaussX
                    ...
                    J(jj, ii) = det(dxdxi);
                    cR = PHTelem{patchIndex}(i).modifiedC * squeeze(B_uv(ii,jj,:));
                    
                    dRdx = PHTelem{patchIndex}(i).modifiedC * squeeze(dBdu(ii,jj,:))*2/(xmax-xmin);
                    dRdy = PHTelem{patchIndex}(i).modifiedC * squeeze(dBdv(ii,jj,:))*2/(ymax-ymin);
                    dR = [dRdx';dRdy'];
