function  plotErrorPHTElasticPlateHoleC1( sol0, PHTelem, GIFTmesh, p, q, Cmat, rad, tx )
        ...
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    cR = PHTelem{patchIndex}(i).modifiedC * squeeze(Buv(ii,jj,:));
                    cdRdx = PHTelem{patchIndex}(i).modifiedC * squeeze(dBdu(ii,jj,:))*2/(xmax-xmin);
                    cdRdy = PHTelem{patchIndex}(i).modifiedC * squeeze(dBdv(ii,jj,:))*2/(ymax-ymin);
