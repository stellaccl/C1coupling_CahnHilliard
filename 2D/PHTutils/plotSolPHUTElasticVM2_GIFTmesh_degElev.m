function  plotSolPHUTElasticVM2_GIFTmesh_degElev( sol0, GIFTmesh,PHUTelem, p, q, Cmat)
...
%define the 2D Bernstein polynomials for PHUTelemDE
[B_u_DE, dB_u_DE] = bernstein_basis(xi,p+1);
[B_v_DE, dB_v_DE] = bernstein_basis(eta,q+1);

Buv_DE = zeros(numPts, numPts, numPts*numPts);
dBdu_DE = zeros(numPts, numPts, numPts*numPts);
dBdv_DE = zeros(numPts, numPts, numPts*numPts);
        ...
        if isempty(PHUTelem{indexPatch}(i).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    for t=1:nument
                        if PHUTelem{indexPatch}(i).polyDegree(t)==p
                            cR(t) = PHUTelem{indexPatch}(i).modifiedC(t,1:(p+1)*(q+1)) * squeeze(Buv(ii,jj,:));
                            cdRdx(t) = PHUTelem{indexPatch}(i).modifiedC(t,1:(p+1)*(q+1)) * squeeze(dBdu(ii,jj,:))*2/(xmax-xmin);
                            cdRdy(t) = PHUTelem{indexPatch}(i).modifiedC(t,1:(p+1)*(q+1)) * squeeze(dBdv(ii,jj,:))*2/(ymax-ymin);
                        else
                            cR(t) = PHUTelem{indexPatch}(i).modifiedC(t,:) * squeeze(Buv_DE(ii,jj,:));
                            cdRdx(t) = PHUTelem{indexPatch}(i).modifiedC(t,:) * squeeze(dBdu_DE(ii,jj,:))*2/(xmax-xmin);
                            cdRdy(t) = PHUTelem{indexPatch}(i).modifiedC(t,:) * squeeze(dBdv_DE(ii,jj,:))*2/(ymax-ymin);
                        end
                    end
