function  plotPHTShellMeshMP_multiElemGIFTmesh( PHTelem, GIFTmesh )
% plots the PHT mesh for the given shell geometry
% for GIFTmesh with multiple elem on u and v direction
    ...
    for i=1:length(PHTelem{patchIndex})
        ...
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for j=1:numPts
                ...
                coord_south(j,:) = paramMapShell_multiElemGIFTmesh(i,GIFTmesh{patchIndex}, uref(j), -1, pxmin, pymin, pxmax, pymax );
                coord_east(j,:) = paramMapShell_multiElemGIFTmesh(i,GIFTmesh{patchIndex}, 1, vref(j), pxmin, pymin, pxmax, pymax );
                coord_north(j,:) = paramMapShell_multiElemGIFTmesh(i,GIFTmesh{patchIndex}, -uref(j), 1, pxmin, pymin, pxmax, pymax );
                coord_west(j,:) = paramMapShell_multiElemGIFTmesh(i,GIFTmesh{patchIndex}, -1, -vref(j), pxmin, pymin, pxmax, pymax );
            ...
            %coord_mid = paramMapShell(GIFTmesh{patchIndex}, 0, 0, pxmin, pymin, pxmax, pymax );
              coord_mid = paramMapShell_multiElemGIFTmesh(i,GIFTmesh{patchIndex}, 0, 0, pxmin, pymin, pxmax, pymax );
