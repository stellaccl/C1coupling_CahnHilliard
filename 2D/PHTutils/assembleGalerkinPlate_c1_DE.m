function [ stiff, rhs ] = assembleGalerkinPlate_c1_DE( PHTelem, GIFTmesh, sizeBasis, p, q, Cmat, q0 )
...
%%%%%% --------------------------------------
%for degree elevated basis
[B_u_DE, dB_u_DE,ddB_u_DE] = bernstein_basis(gauss_coord_x,p+1);
[B_v_DE, dB_v_DE,ddB_v_DE] = bernstein_basis(gauss_coord_y,q+1);

Buv_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
dBdu_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
dBdv_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
d2Bdu_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
d2Bdv_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
d2Bdudv_DE = zeros(ngauss_x, ngauss_y, (p+2)*(q+2));
...
for j=1:q+2
    for i=1:p+2
        ...
        dBdu_DE(:,:,basisCounter) = dB_u_DE(:,i)*B_v_DE(:,j)';
        dBdv_DE(:,:,basisCounter) = B_u_DE(:,i)*dB_v_DE(:,j)';
        d2Bdu_DE(:,:,basisCounter) = ddB_u_DE(:,i)*B_v_DE(:,j)';
        d2Bdv_DE(:,:,basisCounter) = B_u_DE(:,i)*ddB_v_DE(:,j)';
        d2Bdudv_DE(:,:,basisCounter) = dB_u_DE(:,i)*dB_v_DE(:,j)';
    end
end
%%%%%% --------------------------------------
                ...
                for ii=1:ngauss_x
                    ...
                    for t=1:nument
                        ...
                        if PHTelem{patchIndex}(i).polyDegree(t)==p
                            ...
                            R = (PHTelem{patchIndex}(i).modifiedC(t,1:(p+1)*(q+1)))*squeeze(Buv(ii,jj,:));
                            ...
                            d2Rdx = (PHTelem{patchIndex}(i).modifiedC(t,1:(p+1)*(q+1)))*squeeze(d2Bdu(ii,jj,:));
                            d2Rdy = (PHTelem{patchIndex}(i).modifiedC(t,1:(p+1)*(q+1)))*squeeze(d2Bdv(ii,jj,:));
                            d2Rdxdy = (PHTelem{patchIndex}(i).modifiedC(t,1:(p+1)*(q+1)))*squeeze(d2Bdudv(ii,jj,:));
                            
                        else
                            
                            R = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(Buv_DE(ii,jj,:));
                            dRdx = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(dBdu_DE(ii,jj,:));
                            dRdy = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(dBdv_DE(ii,jj,:));
                            d2Rdx = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(d2Bdu_DE(ii,jj,:));
                            d2Rdy = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(d2Bdv_DE(ii,jj,:));
                            d2Rdxdy = (PHTelem{patchIndex}(i).modifiedC(t,:))*squeeze(d2Bdudv_DE(ii,jj,:));
