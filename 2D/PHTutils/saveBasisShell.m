function [ basisFun ] = saveBasisShell( PHTelem,GIFTmesh, p, q )
...
%sdR_temp = cell(numPatches, numElements, ngauss_x, ngauss_y);
        ...
        if isempty(PHTelem{patchIndex}(elemIndex).children)
            ...
            for jj=1:ngauss_y
                for ii=1:ngauss_x
                    ...
                    [ ~, dxdxi, d2xdxi2, dxdxi2] = paramMapShell(GIFTmesh{patchIndex}, uref,vref, xmin, ymin, xmax, ymax);
                    ...
                    % dR = dxdxi2\(ddR - d2xdxi2*dR);
                    
                    sdR{patchIndex,elemIndex,ii,jj} =dR;
                    
                    % sd2Rdxdy{patchIndex,elemIndex,ii,jj} =ddR(2,:);
                    
                    %[laplacian,ddRx,ddRy] = computeLaplaceBeltrami(G,dxdxi,d2xdxi2,dRdx,dRdy,d2Rdx,d2Rdy,d2Rdxdy);
                    ddRx = d2Rdx;
                    ddRy = d2Rdy;
                    [laplacian] = computeLaplaceBeltrami(G,dxdxi,d2xdxi2,dRdx,dRdy,d2Rdx,d2Rdy,d2Rdxdy);
                    sd2Rdx{patchIndex,elemIndex,ii,jj} =ddRx;
                    sd2Rdy{patchIndex,elemIndex,ii,jj} =ddRy;
                    ...
                    sd2Rdxdy{patchIndex,elemIndex,ii,jj} =d2Rdxdy;
