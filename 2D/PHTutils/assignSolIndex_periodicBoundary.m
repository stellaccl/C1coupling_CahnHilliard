function [ PHUTelem,numConstraints ] = assignSolIndex_periodicBoundary( PHUTelem,patchBoundaries,p,q )
%with preriodic boundary for two patch bilinear CahnHilliard
...
for indexPatch=1:length(PHUTelem)
    for indexElem=1:length(PHUTelem{indexPatch})
        if isempty(PHUTelem{indexPatch}(indexElem).children)
            PHUTelem{indexPatch}(indexElem).solIndex=zeros(1,(p+1)*(q+1));
        end
    end
end
