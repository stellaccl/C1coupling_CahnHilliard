function  plotPHTMesh_solIndexShell( PHUTelem,GIFTmesh,p)
    ...
    for elemIndex=1:length(PHUTelem{patchIndex})
        if isempty(PHUTelem{patchIndex}(elemIndex).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    count=count+1;
                    [coord, ~] = paramMapShell(GIFTmesh{patchIndex},  uref(ii),vref(jj), pxmin, pymin, pxmax, pymax);
                    %coord=paramMap( GIFTmesh{patchIndex},uref(ii),vref(jj), xmin, ymin, xmax, ymax);
                    text(coord(1), coord(2),coord(3),num2str(solIndex(count)), 'Color', colorArray{patchIndex})
                end
            end
            ...
            for ii=1:numPts
                ...
                south =paramMapShell( GIFTmesh{patchIndex},urefEdge(ii),vrefEdge(1), pxmin, pymin, pxmax, pymax);
                coord_south(ii,:) =[south(1),south(2),south(3)];
                
                north=paramMapShell( GIFTmesh{patchIndex},urefEdge(ii),vrefEdge(end), pxmin, pymin, pxmax, pymax);
                coord_north(ii,:) =[north(1),north(2),north(3)];
                
                east= paramMapShell( GIFTmesh{patchIndex},urefEdge(end),vrefEdge(ii), pxmin, pymin, pxmax, pymax);
                coord_east(ii,:) =[east(1),east(2),east(3)];
                
                west= paramMapShell( GIFTmesh{patchIndex},urefEdge(1),vrefEdge(ii), pxmin, pymin, pxmax, pymax);
                coord_west(ii,:) =[west(1),west(2),west(3)];
                
            end
            
            line(coord_south(:,1),coord_south(:,2),coord_south(:,3),'Color', colorArray{patchIndex}, 'LineWidth',2)
            line(coord_east(:,1),coord_east(:,2),coord_east(:,3),'Color', colorArray{patchIndex}, 'LineWidth',2)
            line(coord_north(:,1),coord_north(:,2),coord_north(:,3),'Color', colorArray{patchIndex}, 'LineWidth',2)
            line(coord_west(:,1),coord_west(:,2),coord_west(:,3),'Color', colorArray{patchIndex}, 'LineWidth',2)
            hold on
            axis equal
            drawnow
