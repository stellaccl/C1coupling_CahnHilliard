function  plotTotalSupportOfType2basis( PHUTelem,GIFTmesh,p,q,type2Basis)
...
numType2=length(type2Basis);

allR=zeros(numPatches,numElem,numPts,numPts);
basisCount=0;
                ...
                if ismember(indexBasis,PHUTelem{indexPatch}(indexElem).nodesGlobal)
                    ...
                    for jj=1:numPts
                        for ii=1:numPts
                            ...
                            tempR = (PHUTelem{indexPatch}(indexElem).modifiedC(localIndex,:))*squeeze(R(ii,jj,:));
                            [ coord, dxdxi] = paramMap( GIFTmesh{indexPatch}, uref(ii), vref(jj), xmin, ymin, xmax, ymax);
                            
                            
                            allR(indexPatch,indexElem,jj,ii)=allR(indexPatch,indexElem,jj,ii)+abs(tempR);
        ...
        if isempty(PHUTelem{indexPatch}(indexElem).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    coordX(jj,ii) = coord(1);
                    coordY(jj,ii) = coord(2);
                    plotR(jj,ii)=allR(indexPatch,indexElem,jj,ii);
                    
                end
            end
            hold on
            surf( coordX,coordY,plotR,'FaceColor','interp','EdgeColor','none')%,'EdgeColor','none','FaceLighting','phong')
