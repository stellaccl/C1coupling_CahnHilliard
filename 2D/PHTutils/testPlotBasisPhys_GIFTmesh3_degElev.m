function  testPlotBasisPhys_GIFTmesh3_degElev( PHUTelem,GIFTmesh,p,q,indexBasis)
            ...
            if ismember(indexBasis,PHUTelem{indexPatch}(indexElem).nodesGlobal)
                ...
                for jj=1:numPts
                    for ii=1:numPts
                        ...
                        if PHUTelem{indexPatch}(indexElem).polyDegree(localIndex)==p
                        ...
                        dFloc(1,:) = dFloc(1,:)*2/(xmax-xmin);
                        dFloc(2,:) = dFloc(2,:)*2/(ymax-ymin);
