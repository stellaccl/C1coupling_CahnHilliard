function [stiff, rhs ] = assembleBiharmonic_c1_degElev(PHTelem,GIFTmesh, sizeBasis, p, q )
%use nodesGlobal
%use modifiedC
...
for patchIndex = 1:length(PHTelem)
    for elemIndex=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(elemIndex).children)
          ...
          %  scrtx = PHTelem{patchIndex}(elemIndex).C1NodesGlobal(1:nument);
            ...
            for jj=1:ngauss_y
                for ii=1:ngauss_x
                    ...
                    d4udx4=24*((coords(2)-1)^2)*(coords(2)^2);
                    d4udy4=24*((coords(1)-1)^2)*(coords(1)^2);
                    d4udx2dy2=4*(6*(coords(1)^2)-6*coords(1)+1)*(6*(coords(2)^2)-6*coords(2)+1);
