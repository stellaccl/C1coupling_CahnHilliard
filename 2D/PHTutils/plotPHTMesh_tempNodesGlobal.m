function  plotPHTMesh_tempNodesGlobal( PHUTelem,GIFTmesh,p)
...
for patchIndex=1:length(PHUTelem)
    ...
    for elemIndex=1:length(PHUTelem{patchIndex})
        if isempty(PHUTelem{patchIndex}(elemIndex).children)
            ...
            count=0;
            nodesGlobal=PHUTelem{patchIndex}(elemIndex).tempNodesGlobal;
            for jj=1:numPts
                for ii=1:numPts
                    count=count+1;
                    [ coord, ~] = paramMap( GIFTmesh{patchIndex}, uref(ii),vref(jj), xmin, ymin, xmax, ymax);
                    %coord=paramMapSphere3(PHUTelem, patchIndex,elemIndex,uref(ii),vref(jj));
                   text(coord(1), coord(2),num2str(nodesGlobal(count)), 'Color', colorArray{colorIndex})
